...
impl Neighbor {
    pub fn new(d_id: DataId, distance: f32, p_id: PointId) -> Neighbor {
        Neighbor {
            d_id,
            distance,
            p_id,
...
impl<T: Clone + Send + Sync> Point<T> {
    ...
    // returns for each layer, a vector Neighbor of a point, one vector by layer
    //
    pub fn get_neighborhood_id(&self) -> Vec<Vec<Neighbor>> {
        let ref_neighbours = self.neighbours.read();
        let nb_layer = ref_neighbours.len();
        let mut neighborhood = Vec::<Vec<Neighbor>>::with_capacity(nb_layer);
        for i in 0..nb_layer {
            let mut neighbours = Vec::<Neighbor>::new();
            let nb_ngbh = ref_neighbours[i].len();
            if nb_ngbh > 0usize {
                neighbours.reserve(nb_ngbh);
                for pointwo in &ref_neighbours[i] {
                    neighbours.push(Neighbor::new(
...
impl<'a, T: Clone + Send + Sync> Iterator for IterPoint<'a, T> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if (self.slot_in_layer as usize)
            < self.point_indexation.points_by_layer.read()[self.layer as usize].len()
        {
            let slot = self.slot_in_layer as usize;
            self.slot_in_layer += 1;
            return Some(
                self.point_indexation.points_by_layer.read()[self.layer as usize][slot].clone(),
            );
        } else {
            self.slot_in_layer = 0;
            self.layer += 1;
...
impl<T: Clone + Send + Sync, D: Distance<T> + Send + Sync> Hnsw<T, D> {
    ...
    fn search_general(&self, data: &Vec<T>, knbn: usize, ef_arg: usize) -> Vec<Neighbor> {
        //
        let mut entry_point;
        {
        ...
        let knn_neighbours: Vec<Neighbor> = neighbours[0..last]
            .iter()
            .map(|p| {
                Neighbor::new(
    ...
    /// search the first knbn nearest neigbours of a data and returns a Vector of Neighbor.   
    /// The parameter ef controls the width of the search in the lowest level, it must be greater
    /// than number of neighbours asked.  
    /// A rule of thumb could be between knbn and max_nb_connection.
    pub fn search(&self, data: &Vec<T>, knbn: usize, ef_arg: usize) -> Vec<Neighbor> {
        //
        let entry_point;
        {
            // a lock on an option an a Arc<Point>
    ...
    pub fn parallel_search(
        ...
        let req_res: Vec<(usize, Vec<Neighbor>)> = receiver.iter().collect();
        let mut answers = Vec::<Vec<Neighbor>>::with_capacity(datas.len());
